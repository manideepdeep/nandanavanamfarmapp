...
BEGIN
  IF new.raw_user_meta_data->>'avatar_url' IS NULL OR new.raw_user_meta_data->>'avatar_url' = '' THEN
    new.raw_user_meta_data = jsonb_set(new.raw_user_meta_data, '{avatar_url}', '"https://teamup.org/wp-content/uploads/2024/05/member.png"'::jsonb);
  END IF;
...
CREATE POLICY "Enable Update for auth users" ON "public"."users" FOR UPDATE TO "authenticated" USING (true) WITH CHECK (true);



CREATE POLICY "Enable Updates for Admins only" ON "public"."category" FOR UPDATE TO "authenticated" USING ((( SELECT "users"."type"
   FROM "public"."users"
...
CREATE POLICY "Enable update for users" ON "public"."product" FOR UPDATE TO "authenticated" USING (true) WITH CHECK (true);
